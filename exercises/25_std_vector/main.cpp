...
int main(int argc, char **argv) {
    constexpr auto VECRTOR_SIZE = sizeof(std::vector<int>);

    {
        std::vector<int> vec{1, 2, 3, 4, 5};
        ASSERT(vec.size() == 5, "Fill in the correct value.");
        // THINK: `std::vector` 的大小是什么意思？与什么有关？
        ASSERT(sizeof(vec) == VECRTOR_SIZE, "Fill in the correct value.");// depends implementation
        int ans[]{1, 2, 3, 4, 5};
    ...
    }
    {
        ...
        {
            ASSERT(vec.size() == 5, "Fill in the correct value.");
            ASSERT(sizeof(vec) == VECRTOR_SIZE, "Fill in the correct value.");
